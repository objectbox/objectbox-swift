...
class QueryTests: XCTestCase {
    ...
    func testSetParameter_Long_SingleParameter() throws {
        ...
        try query.setParameter(AllTypesEntity.long, to: 200)
        let results = try query.all()
        XCTAssertEqual(results.count, 1)
        XCTAssert(results.contains(where: { $0.aLong == 200 }))

        // Trying 2 param setter throws
        XCTAssertThrowsError(try query.setParameters(AllTypesEntity.long, to: 99, -123))

        // Trying collection param setter throws
        XCTAssertThrowsError(try query.setParameters(AllTypesEntity.long, to: [1]))
    }
    ...
    func testSetParameter_Long_SingleParameterAliased() throws {
        ...
        try query.setParameter("the long", to: 200)
        let results = try query.all()
        XCTAssertEqual(results.count, 1)
        XCTAssert(results.contains(where: { $0.aLong == 200 }))

        // Trying 2 param setter throws
        XCTAssertThrowsError(try query.setParameters("the long", to: 99, -123))

        // Trying collection param setter throws
        XCTAssertThrowsError(try query.setParameters("the long", to: [1]))
    }
    ...
    func testSetParameter_Long_TwoParameters() throws {
        ...
        try query.setParameters(AllTypesEntity.long, to: 90, 300)
        XCTAssertEqual(try query.count(), 1)
        try query.setParameters(AllTypesEntity.long, to: -300, 300)
        let results = try query.all()
        ...
        XCTAssertThrowsError(try query.setParameter(AllTypesEntity.long, to: 200))
    ...
    func testSetParameter_Long_TwoParametersAliased() throws {
        ...
        XCTAssertEqual(try query.count(), 0)
        try query.setParameters("longs", to: 90, 300)
        XCTAssertEqual(try query.count(), 1)
        try query.setParameters("longs", to: -300, 300)
        let results = try query.all()
        XCTAssertEqual(results.count, 2)
        XCTAssert(results.contains(where: { $0.aLong == -100 }))
        XCTAssert(results.contains(where: { $0.aLong == 200 }))
        ...
        // Trying 1 param setter throws
        XCTAssertThrowsError(try query.setParameter("longs", to: 200))

        // Trying collection param setter throws
        XCTAssertThrowsError(try query.setParameters("longs", to: [1]))
    }
    ...
    func testSetParameter_Long_Collection() throws {
        ...
        try query.setParameters(AllTypesEntity.long, to: [90, 300])
        XCTAssertEqual(try query.count(), 0)
        try query.setParameters(AllTypesEntity.long, to: [-100, 200])
        let results = try query.all()
        XCTAssertEqual(results.count, 2)
        XCTAssert(results.contains(where: { $0.aLong == -100 }))
        XCTAssert(results.contains(where: { $0.aLong == 200 }))
        ...
        // Trying 2 param setter throws
        XCTAssertThrowsError(try query.setParameters(AllTypesEntity.long, to: 90, 300))
    }
    ...
    func testSetParameter_Long_CollectionAliased() throws {
        ...
        XCTAssertEqual(try query.count(), 1)
        try query.setParameters("longzz", to: [90, 300])
        XCTAssertEqual(try query.count(), 0)
        try query.setParameters("longzz", to: [-100, 200])
        let results = try query.all()
        XCTAssertEqual(results.count, 2)
        XCTAssert(results.contains(where: { $0.aLong == -100 }))
        XCTAssert(results.contains(where: { $0.aLong == 200 }))
        ...
        // Trying 1 param setter throws
        XCTAssertThrowsError(try query.setParameter("longzz", to: 200))

        // Trying 2 param setter throws
        XCTAssertThrowsError(try query.setParameters("longzz", to: 90, 300))
    }
    ...
    func testSetParameter_Integer_SingleParameter() throws {
        ...
        try query.setParameter(AllTypesEntity.integer, to: 200)
        let results = try query.all()
        XCTAssertEqual(results.count, 1)
        XCTAssert(results.contains(where: { $0.integer == 200 }))

        // Trying 2 param setter throws
        XCTAssertThrowsError(try query.setParameters(AllTypesEntity.integer, to: 99, -123))

        // Trying collection param setter throws
        XCTAssertThrowsError(try query.setParameters(AllTypesEntity.integer, to: [1]))
    }
    ...
    func testSetParameter_Integer_SingleParameterAliased() throws {
        ...
        try query.setParameter("an int", to: 200)
        let results = try query.all()
        XCTAssertEqual(results.count, 1)
        XCTAssert(results.contains(where: { $0.integer == 200 }))

        // Trying 2 param setter throws
        XCTAssertThrowsError(try query.setParameters("an int", to: 99, -123))

        // Trying collection param setter throws
        XCTAssertThrowsError(try query.setParameters("an int", to: [1]))
    }
    ...
    func testSetParameter_Integer_TwoParameters() throws {
        ...
        XCTAssertEqual(try query.count(), 0)
        try query.setParameters(AllTypesEntity.integer, to: 90, 300)
        XCTAssertEqual(try query.count(), 1)
        try query.setParameters(AllTypesEntity.integer, to: -300, 300)
        let results = try query.all()
        XCTAssertEqual(results.count, 2)
        ...
        // Trying 1 param setter throws
        XCTAssertThrowsError(try query.setParameter(AllTypesEntity.integer, to: 200))
    ...
    func testSetParameter_Integer_TwoParametersAliased() throws {
        ...
        XCTAssertEqual(try query.count(), 0)
        try query.setParameters("twintegers", to: 90, 300)
        XCTAssertEqual(try query.count(), 1)
        try query.setParameters("twintegers", to: -300, 300)
        let results = try query.all()
        XCTAssertEqual(results.count, 2)
        XCTAssert(results.contains(where: { $0.integer == -100 }))
        XCTAssert(results.contains(where: { $0.integer == 200 }))
        ...
        // Trying 1 param setter throws
        XCTAssertThrowsError(try query.setParameter("twintegers", to: 200))

        // Trying collection param setter throws
        XCTAssertThrowsError(try query.setParameters("twintegers", to: [1]))
    }
    ...
    func testSetParameter_Integer_Collection() throws {
        ...
        try query.setParameters(AllTypesEntity.integer, to: [90, 300])
        XCTAssertEqual(try query.count(), 0)
        try query.setParameters(AllTypesEntity.integer, to: [-100, 200])
        let results = try query.all()
        XCTAssertEqual(results.count, 2)
        XCTAssert(results.contains(where: { $0.integer == -100 }))
        XCTAssert(results.contains(where: { $0.integer == 200 }))
        ...
        // Trying 2 param setter throws
        XCTAssertThrowsError(try query.setParameters(AllTypesEntity.integer, to: 90, 300))
    }
    ...
    func testSetParameter_Integer_CollectionAliased() throws {
        ...
        XCTAssertEqual(try query.count(), 1)
        try query.setParameters("collectintegers", to: [90, 300])
        XCTAssertEqual(try query.count(), 0)
        try query.setParameters("collectintegers", to: [-100, 200])
        let results = try query.all()
        XCTAssertEqual(results.count, 2)
        XCTAssert(results.contains(where: { $0.integer == -100 }))
        XCTAssert(results.contains(where: { $0.integer == 200 }))
        ...
        // Trying 1 param setter throws
        XCTAssertThrowsError(try query.setParameter("collectintegers", to: 200))

        // Trying 2 param setter throws
        XCTAssertThrowsError(try query.setParameters("collectintegers", to: 90, 300))
    }
    ...
    func testSetParameter_Double_Equality() throws {
        ...
        XCTAssertFalse(try query.all().isEmpty)
        try query.setParameters(AllTypesEntity.double, to: 6.0, 0.5) // 0.5 ... 6.0
        XCTAssertEqual(try query.all().count, 2)
        try query.setParameters(AllTypesEntity.double, to: 5.0, 6.0) // 5.0 ... 6.0
        let results = try query.all()
        XCTAssertEqual(results.count, 1)
        XCTAssert(results.contains(where: { $0.aDouble == 5.5 }))

        // Trying 1 param setter throws
        XCTAssertThrowsError(try query.setParameter(AllTypesEntity.double, to: 5.5))
    }
    ...
    func testSetParameter_Double_EqualityAliased() throws {
        ...
        XCTAssertFalse(try query.all().isEmpty)
        try query.setParameters("a double", to: 6.0, 0.5) // 0.5 ... 6.0
        XCTAssertEqual(try query.all().count, 2)
        try query.setParameters("a double", to: 5.0, 6.0) // 5.0 ... 6.0
        let results = try query.all()
        XCTAssertEqual(results.count, 1)
        XCTAssert(results.contains(where: { $0.aDouble == 5.5 }))

        // Trying 1 param setter throws
        XCTAssertThrowsError(try query.setParameter("a double", to: 5.5))
    }
    ...
    func testSetParameter_Double_SingleParameter() throws {
        ...
        XCTAssertEqual(try query.all().count, 2)
        try query.setParameter(AllTypesEntity.double, to: 3.0)
        XCTAssertEqual(try query.all().count, 1)

        // Trying 2 param setter throws
        XCTAssertThrowsError(try query.setParameters(AllTypesEntity.double, to: 1.0, 6.0))
    }
    ...
    func testSetParameter_Double_SingleParameterAliased() throws {
        ...
        XCTAssertEqual(try query.all().count, 2)
        try query.setParameter("single double", to: 3.0)
        XCTAssertEqual(try query.all().count, 1)

        // Trying 2 param setter throws
        XCTAssertThrowsError(try query.setParameters("single double", to: 1.0, 6.0))
    }
    ...
    func testSetParameter_String_SingleParameter() throws {
        ...
        XCTAssertEqual(try query.count(), 1)
        try query.setParameter(AllTypesEntity.string, to: "A")
        XCTAssertEqual(try query.count(), 2)

        // Trying collection param setter throws
        XCTAssertThrowsError(try query.setParameters(AllTypesEntity.string, to: ["x"]))
    }
    ...
    func testSetParameter_String_SingleParameterAliased() throws {
        ...
        XCTAssertEqual(try query.count(), 1)
        try query.setParameter("a string", to: "A")
        XCTAssertEqual(try query.count(), 2)

        // Trying collection param setter throws
        XCTAssertThrowsError(try query.setParameters("a string", to: ["x"]))
    }
    ...
    func testSetParameter_String_Collection() throws {
        ...
        XCTAssertEqual(try query.count(), 1)
        try query.setParameters(AllTypesEntity.string, to: ["Shmoo", "Boo"])
        XCTAssertEqual(try query.count(), 0)
        try query.setParameters(AllTypesEntity.string, to: ["Foo", "Bar"])
        XCTAssertEqual(try query.count(), 2)

        // Trying 1 param setter throws
        XCTAssertThrowsError(try query.setParameter(AllTypesEntity.string, to: "x"))
    }
    ...
    func testSetParameter_String_CollectionAliased() throws {
        ...
        XCTAssertEqual(try query.count(), 1)
        try query.setParameters("string coll", to: ["Shmoo", "Boo"])
        XCTAssertEqual(try query.count(), 0)
        try query.setParameters("string coll", to: ["Foo", "Bar"])
        XCTAssertEqual(try query.count(), 2)

        // Trying 1 param setter throws
        XCTAssertThrowsError(try query.setParameter("string coll", to: "x"))
    }
    ...
    func testSetParameter_MultipleAliases() throws {
        ...
        XCTAssertEqual(try query.count(), 0)
        try query.setParameter("firstLetter", to: "Z")
        XCTAssertEqual(try query.count(), 1)
        try query.setParameter("firstLetter", to: "")
        try query.setParameter("minAge", to: 21)
        XCTAssertEqual(try query.count(), 2)
    }
    ...
    func testRemoveQuery() throws {
        ...
        XCTAssertEqual(try personBox.query().build().all().first?.name, "Susan Ivanova")
    }
    ...
    func testFindIdsQuery() throws {
        ...
        let writtenPersonIDs = try personBox.query().build().all()
