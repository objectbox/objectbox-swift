...
internal class AuthorBinding: ObjectBox.EntityBinding {
    ...
    internal func createEntity(entityReader: ObjectBox.FlatBufferReader, store: ObjectBox.Store) -> EntityType {
        ...
        let entity = Author(
            ...
            books: ToMany<Book>.relation(
                            sourceId: EntityId<Author>(entityId.value),
...
extension Book: ObjectBox.EntityInspectable {
    ...
    fileprivate static func buildEntity(modelBuilder: ObjectBox.ModelBuilder) throws {
        ...
        try entityBuilder.addProperty(name: "name", type: PropertyType.string, id: 2, uid: 19712)
...
internal class BookBinding: ObjectBox.EntityBinding {
    ...
    internal func createEntity(entityReader: ObjectBox.FlatBufferReader, store: ObjectBox.Store) -> EntityType {
        ...
        let entity = Book(
            id: entityId, 
            name: entityReader.read(at: 2 + 2 * 2), 
            authors: ToMany<Author>.backlink(
                            sourceBox: store.box(for: ToMany<Author>.ReferencedType.self),
                            targetId: EntityId<Book>(entityId.value),
                            relationId: 1)
        )
        return entity
    }
